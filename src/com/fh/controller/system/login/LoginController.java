...
public class LoginController extends BaseController {
	...
	public Object login()throws Exception{
		...
		pd = this.getPageData();
		String errInfo = "";
		String KEYDATA[] = pd.getString("KEYDATA").replaceAll("qq446029658fh", "").replaceAll("QQ595141158fh", "").split(",fh,");

		if(null != KEYDATA && KEYDATA.length == 2){
//		if(null != KEYDATA && KEYDATA.length == 3){ //TODO 验证码放开检测
			//shiro管理的session
...
//			String code = KEYDATA[2];
			if(1==0){
//			if(null == code || "".equals(code)){ //TODO 验证码放开检测
//				errInfo = "nullcode"; //验证码为空
				...
				pd.put("USERNAME", USERNAME);
				if(1==1){
//				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){//TODO 验证码放开检测
			...
			if (user != null) {
				...
				if(null == session.getAttribute(Const.SESSION_menuList) || ("yes".equals(changeMenu))){
					...
					if("2".equals(session.getAttribute("changeMenu"))){
						...
						menuList = menuList1;
						menuList.addAll(menuList2);//TODO
					}else{
						session.setAttribute(Const.SESSION_menuList, menuList2);
						session.removeAttribute("changeMenu");
						session.setAttribute("changeMenu", "2");
						menuList = menuList2;
						menuList.addAll(menuList1);//TODO
					}
				}else{
