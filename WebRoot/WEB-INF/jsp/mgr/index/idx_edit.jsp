...
<script type="text/javascript">
	...
	//判断编码是否存在
	function has(){
		var ZD_ID = $("#INDEX_ID").val();
		var BIANMA = $("#PATH").val();
		var url = "mgrIndex/has.do?PATH="+PATH+"&INDEX_ID="+INDEX_ID+"&tm="+new Date().getTime();
		$.get(url,function(data){
			if(data=="error"){
				$("#PATH").css("background-color","#D16E6C");
				
				setTimeout("$('#PATH').val('此编码已存在!')",500);
...
<body>
	<form  action="mgrIndex/save.do" name="Form" id="Form" method="post" >
		...
		<table>
			...
			<tr class="info">
				<td><input type="text" name="PATH" id="PATH" placeholder="这里输入路径" value="${pd.PATH }" title="路径"/></td>
			</tr>
			<tr class="info">
